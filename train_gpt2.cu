...
// defines: rope_precompute_cpu, rope_forward, rope_backward
...
typedef struct {
    ...
    // RoPE tables (allocated when using rotary embeddings; NULL = absolute position embeddings)
...
void gpt2_allocate_state(GPT2 *model, int B, int T) {
    ...
    // Allocate and precompute RoPE cos/sin tables (once per model lifetime)
    // head_dim must be even (true for all standard GPT-2/3 sizes)
    int head_dim = model->config.channels / model->config.num_heads;
    assert(head_dim % 2 == 0);
    int half = head_dim / 2;
    size_t rope_table_elems = (size_t)model->config.max_seq_len * half;
    printf0("allocating %zu KiB for RoPE cos/sin tables (maxT=%d, head_dim=%d)\n",
            (rope_table_elems * 2 * sizeof(float)) >> 10, model->config.max_seq_len, head_dim);

    float* cos_cpu = (float*)mallocCheck(rope_table_elems * sizeof(float));
    float* sin_cpu = (float*)mallocCheck(rope_table_elems * sizeof(float));
    rope_precompute_cpu(cos_cpu, sin_cpu, model->config.max_seq_len, head_dim);

    cudaCheck(cudaMalloc((void**)&model->rope_cos, rope_table_elems * sizeof(float)));
    cudaCheck(cudaMalloc((void**)&model->rope_sin, rope_table_elems * sizeof(float)));
    cudaCheck(cudaMemcpy(model->rope_cos, cos_cpu, rope_table_elems * sizeof(float), cudaMemcpyHostToDevice));
    cudaCheck(cudaMemcpy(model->rope_sin, sin_cpu, rope_table_elems * sizeof(float), cudaMemcpyHostToDevice));
    free(cos_cpu);
    free(sin_cpu);
}
...
void gpt_build_from_descriptor(GPT2 *model, const char* descriptor) {
    ...
    // check the valid prexices and dispatch to the right setup function
...
void gpt2_forward(GPT2 *model, const int* inputs, size_t B, size_t T) {
    ...
    encoder_forward(acts.encoded, model->inputs, params.wte, nullptr, B, T, C, main_stream); // RoPE: token embeddings only (no absolute wpe)
    ...
    for (int l = 0; l < L; l++) {
        ...
        attention_forward(l_atty, l_qkvr, l_att, scratch, B, T, C, NH, model->rope_cos, model->rope_sin, main_stream);
        #endif
...
void gpt2_backward_and_reduce(GPT2 *model, int* inputs, const int* targets, int grad_accum_steps, int micro_step) {
    ...
    for (int l = L-1; l >= 0; l--) {
        ...
        attention_backward(dl_bt4c, buffer_b, scratchX, buffer_a, dl_btc, l_qkvr, l_att, B, T, C, NH, model->rope_cos, model->rope_sin, main_stream);
        #endif
        if(model->recompute >= 2) {
    ...
    // RoPE models: no absolute position embeddings -> pass nullptr for dwpe
    encoder_backward(grads.wte, nullptr, scratchX, model->workload_indices, model->bucket_info,
