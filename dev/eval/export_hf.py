...
# -----------------------------------------------------------------------------
# Tensor functions for both bfloat16 (from int16), float16, and normal float32
# All return float32 tensors
...
def tensor_fp16(data_float16, transpose=False):
    if transpose:
        data_float16 = data_float16.transpose(1, 0)
    return torch.tensor(data_float16).to(torch.float32)
...
def convert(filepath, output, push_to_hub=False, out_dtype="bfloat16"):
    ...
    if version not in [3, 5, 6]:
        print(f"Bad version in model file: {version}")
        exit(1)
    ...
    if version == 3:
        ...
        dtype = np.int16          # bf16 stored as int16
    else:  # version 6
        dtype = np.float16
...
def spin(output):
    ...
    model.eval()
    tokens = tokenizer.encode("Once upon a time", return_tensors="pt")
    tokens = tokens.to('cuda')
    output = model.generate(tokens, max_new_tokens=64, do_sample=True, temperature=0.8, repetition_penalty=1.2)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dtype", "-d", help="Output as float32, float16 or bfloat16 (default)", type=str, default="bfloat16")
