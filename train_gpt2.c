...
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-s") == 0 && i + 1 < argc) {
            ...
            T = atoi(argv[++i]);
        } else if (strcmp(argv[i], "-tg") == 0 && i + 1 < argc) {
            genT = atoi(argv[++i]);
    ...
    if (B <= 0 || T <= 0 || genT <= 0) {
        printf("Error: batch size, sequence length, and tokens generated must be positive\n");
        return 1;
    }
    ...
    for (int step = 0; step < train_steps; step++) {
        ...
        int do_val = (val_every == -1) ? (step == train_steps - 1) : (step % val_every == 0);
        if (do_val) {
            float val_loss = 0.0f;
        ...
        int do_gen = (gen_every == -1) ? (step == train_steps - 1) : (step > 0 && step % gen_every == 0);
        if (do_gen) {
